# ============================================================
# VELLA V4 — app.py (SINGLE FILE)
# ============================================================

# ============================================================
# [ER-CFG-000] CFG ROOT OBJECT (OWNER-FRIENDLY)
# ✅ 이 블럭은 "주인님 전용 튜닝 공간"이다.
# ============================================================
...
CFG = {
    ...
    # =====================================================
    # [PRIORITY 5] 상태 기반 제어(STATE 라벨은 "판단"이 아니라 "라벨" / 사용 여부는 CFG)
    # =====================================================
...
# ============================================================
# [ENGINE_RULE]
# [ER-CFG-013] CFG CONSUMPTION CONTRACT
# ============================================================
# - ENGINE_RULE은 CFG를 해석하지 않는다
# - ENGINE_RULE은 CFG 값을 그대로 소비해 행동한다
# - 종목/투자금/리스크 역시 CFG 값만으로 결정된다
# ============================================================
...
# 번호0
# ============================================================
# VELLA V4 — ENGINE_RULE ID STANDARD (LOCK-IN)
# ============================================================
# 목적:
# - V3처럼 "설계-섹션-코어-APP-AWS"가 흩어져서 헤매는 일을 방지
# - 규칙/계약/불변 로직을 ID로 고정 → 추적/리뷰/로그에서 단일 진실 확보
#
# 적용 원칙(절대):
# 1) ID는 ENGINE_RULE에만 붙인다 (HUMAN_NOTE/코멘트/설명에는 NO)
# 2) ID는 "계약(Contract)" 단위로만 붙인다 (함수/인터페이스/예외정책/불변규칙)
# 3) ID는 모듈 내부에서 중복 금지 (전역 유니크)
# 4) 로그/에러 메시지에 ID를 포함할 수 있으면 포함 (디버그 절약)
# 5) CFG는 이미 번호가 있으므로, ENGINE_RULE ID는 CFG 번호와 "매핑"만 한다
#
# ID 포맷:
#   ER-<DOMAIN>-<NNN>
#   DOMAIN 예: SYM, FEED, IND, ENTRY, EXEC, REC, STATE, CFG, LOOP, RISK, LOG, HEALTH
#
# 예:
#   ER-SYM-001  / ER-FEED-010 / ER-EXEC-020 / ER-REC-030
# ============================================================
...
# ============================================================
# [ENGINE_RULE] HOW TO USE (COPY-PASTE STYLE)
# ============================================================
# 예시:
# def resolve_symbol(cfg):
#     # [ENGINE_RULE][ER-SYM-004] 0_TRADE_SYMBOL empty => raise
#     ...
#
# def place_order(...):
#     # [ENGINE_RULE][ER-EXEC-021] symbol=base, market=market
#     ...
#
# class DataFeed:
#     # [ENGINE_RULE][ER-FEED-011] buffer key uses symbol_info.base
#     ...
#
# def write_event_log(event):
#     # [ENGINE_RULE][ER-LOG-001.a] write-only event trace
#     ...
# ============================================================
...
# ============================================================
# [ENGINE_RULE] CFG CONSUMPTION CONTRACT
# ============================================================
# [ER-CFG-013]
# - ENGINE_RULE은 CFG를 해석하지 않는다
# - ENGINE_RULE은 CFG 값을 그대로 소비해 행동한다
...
# 번호2
# ============================================================
# CFG_MANAGER — V4 (CONFIG LOADER / VALIDATOR)
# ENGINE_RULE / HUMAN_NOTE SPLIT TEMPLATE
# ============================================================
# 원칙:
# - CFG는 단일 진실이다
# - 누락/오타/타입 오류는 "로드 시점"에 차단한다
# - 모듈들은 CFG를 신뢰하고 읽기만 한다
# - (옵션) 핫리로드는 cfg 파일 교체만으로 가능하게 설계
# ============================================================
...
# ============================================================
# 역할:
# - CFG에 저장된 "사람용 종목 표기"를
# - 엔진/거래소 API가 쓰는 "표준 종목 정보"로 변환
# - 이 모듈 출력만 전 모듈이 공통으로 사용 (단일 진실)
# ============================================================
...
# ============================================================
# [ENGINE_RULE]
# [ER-SYM-005] CONSUMPTION CONTRACT
# ============================================================
# - app.py / MAIN_LOOP은 시작 시 1회 resolve_symbol(cfg) 실행
# - 이후 모든 모듈은 symbol(str) 대신 SymbolInfo를 받는다
# - DATA_FEED / EXECUTION / RECONCILER는 symbol_info.base 사용
# - 로깅/표시는 symbol_info.display 사용
# ============================================================
...
# 번호4
# ============================================================
# ENTRY_ENGINE_RULE — V4 (EXECUTION PIPELINE)
# ============================================================
# 원칙:
# - 이 모듈은 판단하지 않는다
# - CFG를 그대로 소비해 행동만 한다
# - 구조 고정, 의미/강도는 CFG가 결정
# ============================================================

# ============================================================
# [ENGINE_RULE]
# [ER-ENTRY-001] DEPENDENCIES CONTRACT
# ============================================================
# - CFG: tuning-only config dict
# - state: 엔진 상태 객체
# - candle: 현재 봉 (open, high, low, close, ts)
# - ema_value: EMA 값 (CFG["4_EMA_PERIOD"] 기반)
# ============================================================
...
#번호5
# ============================================================
# EXIT_ENGINE_RULE — V4 (EXECUTION DECISION PIPELINE)
# ENGINE_RULE / HUMAN_NOTE SPLIT TEMPLATE
# ============================================================
# 원칙:
# - 이 모듈은 "나갈지/얼마나 나갈지"만 결정한다
# - 주문 실행은 EXECUTION_MODULE에 위임한다
# - 전략 성격/강도/결과는 CFG 조합으로만 결정된다
# - 구조 고정, 의미/강도는 CFG가 결정
# ============================================================

# ============================================================
# [ENGINE_RULE]
# [ER-EXIT-001] DEPENDENCIES CONTRACT
# ============================================================
# - cfg: V4 CFG dict (EXIT 관련 키 포함)
# - state: 포지션 상태 객체
# - candles3 / candles5: 컨텍스트 캔들
...
# 원칙:
# - 종목/거래소 하드코딩 금지
# - 이 모듈은 "캔들"만 제공한다 (판단/주문/전략 모름)
# - 단일 진실: 완료봉/진행중봉 구분은 여기서만
# - REST 스냅샷 + (옵션) WS 스트림, 인터페이스 동일
# ============================================================
...
# ============================================================
# [ENGINE_RULE]
# [ER-FEED-001] SYMBOL CONTRACT
# ============================================================
# - resolve_symbol(cfg) 결과(SymbolInfo)를 인자로 받는다
# - 거래소/API 호출에는 반드시 symbol_info.base 사용
# ============================================================
...
#번호8
# ============================================================
# STATE_MODULE — V4 (LABELER ONLY)
# ============================================================
# 원칙:
# - 이 모듈은 판단/주문/엔트리를 모른다
# - 시장 상태를 "라벨"로만 제공한다
# - 의미/강도는 CFG가 결정한다
# ============================================================
...
# ============================================================
# [ENGINE_RULE]
# [ER-STATE-001] DEPENDENCIES CONTRACT
# ============================================================
# - cfg: CFG dict
# - candles: 최근 봉 리스트 (oldest -> newest)
# - ema_series: EMA 값 리스트 (candles와 정렬)
# ============================================================
...
# ============================================================
# 원칙:
# - 상태는 "사실"만 저장한다 (의미/해석 없음)
# - 엔진은 재시작되어도 상태로부터 복구된다
# - 저장은 원자적(atomic)으로 수행한다
# - 전략 성격은 CFG가 결정, 상태는 결과만 보관
# ============================================================
...
#번호10
# ============================================================
# ENTRY MODULE — DESIGN TEMPLATE (V4)
# ============================================================
# 목적:
# - 엔트리 후보는 넓게 수집
# - 전략적 판단은 하지 않는다
# - CFG 기준에 대한 조건 평가만 수행한다
# - 엔진 불변, 모든 강도/스위치는 CFG
# ============================================================
...
#번호11
# ============================================================
# EXECUTION_MODULE — V4 (ORDER ADAPTER ONLY)
# ENGINE_RULE / HUMAN_NOTE SPLIT TEMPLATE
# ============================================================
# 원칙:
# - 이 모듈은 "판단"을 하지 않는다
# - 엔트리/상태/필터를 모른다
# - 주문 요청을 받아 거래소로 전달하고 결과만 돌려준다
# - 실패/예외는 "보고"하고, 재시도는 CFG로만 제어한다
# ============================================================

# ============================================================
# [ENGINE_RULE]
# [ER-EXEC-001] DEPENDENCIES CONTRACT
# ============================================================
# - exchange: 거래소 API 래퍼
# - cfg: CFG dict (실행 안정성 파라미터만)
# - state: 엔진 상태 (로그/감사)
# - symbol_info: 표준 SymbolInfo
# ============================================================
...
#번호13
# ============================================================
# RISK_GUARD_MODULE — V4 (KILL-SWITCH / SAFETY GATE)
# ENGINE_RULE / HUMAN_NOTE SPLIT TEMPLATE
# ============================================================
# 원칙:
# - 막을지/통과시킬지만 결정
# - 전략/판단/주문 금지
# - ENTRY/EXIT 직전 OK/NO 반환
# - 모든 기준은 CFG
# ============================================================
...
# ============================================================
# 원칙:
# - 이 모듈은 "순서와 타이밍"만 책임진다
# - 판단/전략/주문 로직 금지
# - 모든 모듈 호출 순서는 여기서 단일화
# - 예외가 나도 상태는 안전하게 보존
# ============================================================
...
# ============================================================
# [ENGINE_RULE]
# [ER-SCH-001] DEPENDENCIES (INJECTED)
# ============================================================
# - cfg: CFG dict
# - feed: DATA_FEED_MODULE (symbol-agnostic)
# - state: EngineState (STATE_MANAGER)
# - state_mgr: STATE_MANAGER (save/load helpers)
# - entry_engine: ENTRY_ENGINE_RULE
# - exit_engine: EXIT_ENGINE_RULE
# - execution: EXECUTION_MODULE
# - state_labeler: STATE_MODULE
# - ema_calc: EMA calculator util
# - clock: time provider (now_ms)
